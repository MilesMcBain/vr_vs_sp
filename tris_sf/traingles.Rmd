...
First we dertermine roughly how big we want the triangles to be, by working back from a sensible number. 3000 sounds good.

```{r}
# Determine max area setting for around 3000 tris
bb_area <- function(sf_obj){
  bbox <- st_bbox(sf_obj)
  area <- (bbox[3] - bbox[1]) * (bbox[4] - bbox[2])
  area
}
bb_area(nc) / 3000 #0.008
```
